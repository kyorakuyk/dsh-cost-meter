...
function sameRate(left, right) {
	return left.input === right.input && left.output === right.output && (left.cacheRead ?? left.input) === (right.cacheRead ?? right.input) && (left.cacheWrite ?? left.input) === (right.cacheWrite ?? right.input);
}
//#endregion
...
function foldUsage(state, provider, model, usage, resolve, atTime) {
	...
	if (resolved !== void 0) {
		...
		if (resolved.outdated === true) {
			...
			if (resolved.latestRate !== void 0) entry.latestRate = resolved.latestRate;
			if (resolved.latestSource !== void 0) entry.latestSource = resolved.latestSource;
		}
...
function cloneEntry(entry) {
	return {
		...
		...entry.appliedRate === void 0 ? {} : { appliedRate: { ...entry.appliedRate } },
		...entry.priceOutdated === void 0 ? {} : { priceOutdated: entry.priceOutdated },
		...entry.latestRate === void 0 ? {} : { latestRate: { ...entry.latestRate } },
		...entry.latestSource === void 0 ? {} : { latestSource: entry.latestSource }
	};
}
/**
...
function collectOutdated(entries) {
	const outdated = [];
	for (const entry of entries) {
		if (entry.priceOutdated !== true || entry.appliedRate === void 0 || entry.latestRate === void 0 || entry.latestSource === void 0) continue;
		outdated.push({
			provider: entry.provider,
			model: entry.model,
...
function toReport(state) {
	...
	return {
		...
		entries,
		unpriced,
		outdated: collectOutdated(entries)
	};
}
//#endregion
...
function createPriceResolver(options) {
	return (provider, model, atTime) => {
		...
		const snapshotCurrent = options.snapshot?.stale !== true ? snapshot : void 0;
		...
		if (manual !== void 0) {
			...
			return {
				rate: manual.rate,
				source: "manual",
				...manual.window === void 0 ? {} : { window: manual.window },
				...manual.versionFrom === void 0 ? {} : { versionFrom: manual.versionFrom },
				...outdated
			};
		}
...
* known price).
*/
function detectOutdated(manualRate, provider, fetched, snapshot) {
	if (provider === "openrouter" && fetched !== void 0 && !sameRate(manualRate, fetched)) return {
		outdated: true,
		latestRate: fetched,
		latestSource: "openrouter"
	};
	if (snapshot !== void 0 && !sameRate(manualRate, snapshot.rate)) return {
		outdated: true,
		latestRate: snapshot.rate,
...
var CostMeter = class extends Service {
	...
	*/
	collectOutdated() {
		...
		for (const session of this.tracked) for (const entry of toReport(this._sync(session)).outdated) seen.set(`${entry.provider}\u0000${entry.model}`, entry);
		return [...seen.values()];
	}
	/**
